...
To overcome platform limitations, Nextract implements a unique hybrid scheduling approach:
...
This project relies on environment variables for configuration, secrets, and third-party integrations.
Create a `.env` file in the project root for local development, and configure the same variables in your hosting provider (e.g. Vercel) for production.
...
CLERK_WEBHOOK_SIGNING_SECRET=whsec_your-clerk-webhook-secret

# Payments (Stripe)
STRIPE_SECRET_KEY=sk_test_your-stripe-secret-key
STRIPE_WEBHOOK_SECRET=whsec_your-stripe-webhook-secret
STRIPE_SMALL_PACK_PRICE_ID=price_small_123
STRIPE_MEDIUM_PACK_PRICE_ID=price_medium_456
STRIPE_LARGE_PACK_PRICE_ID=price_large_789
...
# Hybrid scheduling (local + backup cron)
...
### Clerk Redirect Configuration (Optional)

These control Clerk authentication redirects.
...
* **Do not set `NODE_ENV` manually**. It is automatically managed by Next.js and hosting providers like Vercel.
* Variables prefixed with `NEXT_PUBLIC_` are exposed to the browser. Never place secrets in them.
* `ENCRYPTION_SECRET` must remain constant once credentials are stored, or existing encrypted data will become unreadable.
* In production, update `NEXT_PUBLIC_APP_URL` to your deployed domain (e.g. `https://your-app.vercel.app`).
* All required variables must be configured in Vercel’s Environment Variables dashboard for successful deployment.
```
...
We welcome contributions to Nextract! Please follow these guidelines:
...
**License**: MIT License  
**Maintainer**: Abdulahad iSmail
**Support**: [Create an issue](https://https://github.com/PheonixCodder/Nextract/issues)