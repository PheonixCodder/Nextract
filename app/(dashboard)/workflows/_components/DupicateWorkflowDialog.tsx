...
const DuplicateWorkflowDialog = ({ workflowId }: { workflowId: string }) => {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: () => {
      ...
      setOpen(false); // Close the dialog directly after success
    },
    onError: (error: Error) => {
      toast.error(`Failed to Duplicate workflow: ${error.message}`, { id: "duplicate-workflow" });
    },
  });
  ...
  return (
    <Dialog open={open} onOpenChange={(open) => {
      ...
      <DialogContent className="px-0">
        <CustomDialogHeader icon={Layers2Icon} title="Duplicate Workflow" subTitle="Start building your workflow" />
        <div className="p-6">
          <Form {...form}>
